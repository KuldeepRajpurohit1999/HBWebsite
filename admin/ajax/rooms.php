      ...
      if($row['status'] == 1){
       ...
       $data.="
         <tr class='align-middle'>
             ...
             <button type='button' onclick=\"room_images($row[id],'$row[name]')\" class='btn btn-info shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#room_images'>
      ...
      while($row = mysqli_fetch_assoc($res))
      {
        ...
        if($row['thumb'] == 1){
          ...
          $thumb_btn = "<button onclick='thumb_image($row[sr_no],$row[room_id])' class='btn btn-secondary btn-sm shadow-none'>
            <i class='bi bi-check-lg fs-5'></td>
          </button>";   
        }
        echo<<<data
            <tr class='align-middle'>
