<?php
    ...
    if(isset($_GET['fetch_rooms']))
    {
       ...
       if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
       {
        ...
        {
            echo"<h3 class='text-center text-danger'>Invalid Dates Entered!</h1>";
            exit;
        }
        ...
        while($room_data = mysqli_fetch_assoc($room_res))
        {
            ...
            while($fac_row = mysqli_fetch_assoc($fac_q)){
                ...
                $facilities_data .="<span class='badge rounded-pill bg-light text-dark text-wrap lh-base me-2 mb-1'>
                $fac_row[name]
                </span>";
            }
            ...
            $fea_q = mysqli_query($con,"SELECT f.name FROM `features` f INNER JOIN `room_features` rfea ON f.id = rfea.features_id 
            WHERE rfea.room_id = '$room_data[id]'");

            $features_data = "";

            while($fea_row = mysqli_fetch_assoc($fea_q)){
                $features_data .="<span class='badge rounded-pill bg-light text-dark text-wrap lh-base me-2 mb-1'>
                $fea_row[name]
                </span>";
            }           
           ...
           $output .="
                <div class='card mb-4 border-0 shadow'>
                    <div class='row g-0 p-3 align-items-center'>
                        <div class='col-md-5 g-1 mb-lg-0 mb-md-0 mb-3'>
                            <img src='$room_thumb' class='img-fluid w-md-200 rounded'>
                        </div>
                        <div class='col-md-5 px-lg-2 px-md-4 px-0'>
                            <h5 class='ms-1 mb-3'>$room_data[name]</h5>
                            <div class='feature ms-1 mb-3'>
                                <h6 class='mb-1'>Features</h6>
                                $features_data
                            </div>
                            <div class='facilities ms-1 mb-3'>
                                <h6 class='mb-1'>Facilities</h6>
                                $facilities_data
                            </div>
                            <div class='guests mb-lg-0 mb-md-0 mb-4'>
                                <h6 class='ms-1 mb-1'>Guests</h6>
        ...
        {
            echo"<h3 class='text-center text-danger'>No rooms to show!</h1>";
        }
    }
?>